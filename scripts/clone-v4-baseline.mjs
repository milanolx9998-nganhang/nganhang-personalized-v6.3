// One-time, non-overwriting clone from the preserved V4 working copy.
...
if(path.basename(root)!=='nganhang-personalized-v5')throw Error('Sai thư mục đích');
const require=createRequire(new URL('../backend/package.json',import.meta.url)),pg=require('pg'),dotenv=require('dotenv');
const source=path.resolve(root,'../nganhang-personalized-v1'),cfg=dotenv.parse(fs.readFileSync(path.join(source,'backend/.env')));
if(cfg.DB_NAME!=='nganhang_personalized_v1'||cfg.DATABASE_URL)throw Error('Cấu hình nguồn khác dự kiến');
const target='nganhang_personalized_v5',connection={host:cfg.DB_HOST,port:cfg.DB_PORT,user:cfg.DB_USER,password:cfg.DB_PASSWORD},db=new pg.Pool({...connection,database:cfg.DB_NAME});
const env={...process.env,PGHOST:cfg.DB_HOST,PGPORT:cfg.DB_PORT||'5432',PGUSER:cfg.DB_USER,PGPASSWORD:cfg.DB_PASSWORD};
const folder=path.join(root,'backups','v4-baseline'),dump=path.join(folder,'database.dump');
