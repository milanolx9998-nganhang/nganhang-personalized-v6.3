...
export default function CurriculumPicker({rows,selected,onChange}){
 ...
 const visible=rows.filter(y=>(y.yccd_code+' '+(y.yccd_label||'')+' '+y.yccd_text+' '+y.outcome_title).toLocaleLowerCase('vi').includes(search.toLocaleLowerCase('vi')));
...
export function CurriculumField({q,onChange}){
 ...
 return <label>YCCĐ chuẩn {loading?'· đang tải…':''}<select value={q.yccd_id||''} onChange={e=>{const y=rows.find(x=>x.id===Number(e.target.value));onChange({...q,yccd_id:y?.id||null,outcome_id:y?.outcome_id||null,branch_id:y?.branch_id||null,yccd_code:y?.yccd_code||'',outcome_code:y?.outcome_code||''});}}><option value="">Chưa gán — chỉ lưu bản nháp</option>{rows.map(y=><option key={y.id} value={y.id}>{rowYccd(y)} · {y.yccd_text}</option>)}</select>{error&&<small role="alert">{error}</small>}{q.yccd_id&&<small>{rows.find(y=>y.id===q.yccd_id)?.yccd_text}</small>}</label>;
}
