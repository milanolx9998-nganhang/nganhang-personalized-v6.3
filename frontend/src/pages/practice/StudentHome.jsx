...
export default function StudentHome(){
 const {user}=useAuth(),load=usePortfolio(base+'/students/'+user.id+'/portfolio'),nav=useNavigate(),[error,setError]=useState(''),[starting,setStarting]=useState(null);
 ...
 const d=load.data;
 // Thứ tự ưu tiên: bài đang dở → bài giao sắp hết hạn (< 48 giờ) → bài giao khác → nội dung củng cố → tự luyện.
 ...
 {urgent&&<article className="practice-card resume-priority urgent-assignment"><span className="eyebrow">{d.unfinished.length?'Tiếp theo':'Ưu tiên tiếp theo'} · {timeLeft(urgent.closes_at)}</span><h2>{urgent.title}</h2><p>Hạn nộp: {date(urgent.closes_at)}</p><button type="button" className={'btn '+(d.unfinished.length?'':'primary')} disabled={starting===urgent.id} onClick={()=>start(urgent)}>{starting===urgent.id?'Đang mở…':'Làm bài'}</button></article>}
 ...
 <article className="practice-card"><h2>Bài được giao</h2><p>{d.assignment_summary.pending} bài chưa hoàn thành · {d.assignment_summary.due_soon} bài đến hạn trong 7 ngày</p>{d.upcoming_assignments.filter(a=>a!==urgent).map(a=><p key={a.id}><strong>{a.title}</strong><br/>{a.closes_at?'Hạn: '+date(a.closes_at)+' · '+timeLeft(a.closes_at):'Không đặt hạn nộp'} <button type="button" className="btn" disabled={starting===a.id} onClick={()=>start(a)}>{starting===a.id?'Đang mở…':'Làm bài'}</button></p>)}<Link to="/practice/assignments">Xem bài được giao →</Link></article>
