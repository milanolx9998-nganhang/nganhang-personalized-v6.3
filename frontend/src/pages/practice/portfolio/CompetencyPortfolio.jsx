...
function ProfileContent({id,subject,grade,mode,self}){
 ...
 if(mode==='knowledge')return <>{!d.items.length&&<p>Chưa có chương trình được công bố cho môn/khối này.</p>}{d.items.map(y=><article className="practice-card" key={y.yccd_id}><h3 title={y.outcome_code}>{y.outcome_label||y.outcome_code} · {y.title}</h3><p><strong title={y.code}>{y.label||y.code}</strong> · {y.text}</p><p>{y.note}</p>{!y.linked_topic_mastery.length?<p>Chưa có minh chứng bài học liên kết.</p>:y.linked_topic_mastery.map((m,i)=><p key={i}>{m.topic_name} · M{m.level}: {percent(m.score)} · Độ tin cậy: {m.confidence}</p>)}</article>)}</>;
