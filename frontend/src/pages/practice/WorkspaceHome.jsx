...
export default function WorkspaceHome(){
 ...
 {!load.data?<LoadState load={load}/>:<><h2>Hôm nay cần chú ý</h2><div className="portfolio-stats decision-stats">{[[load.data.summary.inactive_7d,'Học sinh chưa luyện 7 ngày','/practice'],[load.data.summary.declining,'Học sinh có xu hướng giảm','/practice'],[load.data.summary.assignments_due,'Bài giao đến hạn trong 7 ngày','/practice'],[load.data.summary.questions_pending,'Câu đang chờ duyệt','/practice/banks']].map(([n,label,to])=><article key={label}><strong>{n}</strong><Link to={to}>{label} →</Link></article>)}</div>{!load.data.summary.students&&<p className="scope-empty">Chưa có học sinh trong phạm vi được cấp. Liên hệ quản trị để phân công lớp và môn.</p>}
