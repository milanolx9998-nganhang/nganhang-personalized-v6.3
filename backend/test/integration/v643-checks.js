...
export function registerV643(get){
 ...
 test('V643 UI học sinh tick bài, cờ persist/offline; quản trị và review hiển thị',{timeout:90000},async()=>{
  ...
  try{
   ...
   await page.goto(origin+'/practice/banks?subject_id='+master.subject_id+'&grade=7&topic_id='+a);await page.getByRole('heading',{name:'Kho câu hỏi và duyệt nội dung'}).waitFor();await page.screenshot({path:path.join(dir,'v6_4-bank-lesson-scope.png'),fullPage:true});
   await page.locator('.scope-v2-lesson').filter({hasText:'TEST V643 Bài A'}).getByText('Tinh chỉnh Outcome / YCCĐ',{exact:true}).click();await page.screenshot({path:path.join(dir,'v6_4-bank-outcome-drilldown.png'),fullPage:true});
   await page.goto(origin+'/practice/reviews');await page.getByRole('heading',{name:'Hàng đợi rà soát câu hỏi'}).waitFor();await page.getByRole('button',{name:'Mở hồ sơ và so sánh',exact:true}).first().click();await page.getByText('So sánh và duyệt phiên bản',{exact:true}).waitFor();assert.equal(await page.locator('.review-diff-grid').evaluate(el=>getComputedStyle(el).display),'grid');await page.screenshot({path:path.join(dir,'v6_4-question-version-review.png'),fullPage:true});
