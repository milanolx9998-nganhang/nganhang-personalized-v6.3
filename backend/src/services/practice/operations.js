...
export async function operations(){
 ...
 return {http:{...http},db:dbStats(),cache:{status:redisStatus(),metrics:{...cacheMetrics},redis:await redisInfo()},rate_limit:{...rateLimitMetrics,blocked_by:{...rateLimitMetrics.blocked_by}},hash_workers:passwordWorkerStats(),trust_proxy:process.env.TRUST_PROXY||null,at:new Date().toISOString(),uptime_seconds:process.uptime(),rss_bytes:process.memoryUsage().rss,cpu_microseconds:process.cpuUsage(),disk,connections,backup,backup_age_hours:backup.last_success?(Date.now()-new Date(backup.last_success))/3600000:null};
}
