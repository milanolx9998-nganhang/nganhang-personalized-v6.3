...
export async function resolveMatrixScope(raw,client){
 ...
 const labels=(await client.query('SELECT y.id,y.code,y.text,'+yccdLabelSql('y','o')+' AS label,o.code AS outcome_code,'+outcomeLabelSql('o')+' AS outcome_label,o.title AS outcome_title,o.curriculum_version FROM curriculum_yccds y JOIN curriculum_outcomes o ON o.id=y.outcome_id WHERE y.id=ANY($1::int[]) ORDER BY y.id',[yccd_ids])).rows;
