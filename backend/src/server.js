...
app.get('/api/health', async (_req, res) => {
  try{await pool.query('SELECT 1');await storage.health();res.json({ status: 'ok', database:'ok', storage:'ok',...publicProfile(), timestamp: new Date().toISOString(), version: '6.6.4-pilot' });}catch{res.status(503).json({status:'unavailable'});}
});
...
(async () => {
  try {
    ...
    app.listen(PORT, HOST, () => {
      console.log(`
Ngân hàng V6.6.4 (${profile.name}) đang chạy tại:
