...
r.get('/classes',wrap(async(req,res)=>{if(req.user.role==='student')fail('Không đủ quyền',403);res.json((await pool.query('SELECT c.*,y.name AS school_year FROM classes c JOIN school_years y ON y.id=c.school_year_id WHERE $2 OR c.id=ANY($1::int[]) ORDER BY y.name DESC,c.name',[await classesFor(req.user),req.user.role==='admin'])).rows);}));
