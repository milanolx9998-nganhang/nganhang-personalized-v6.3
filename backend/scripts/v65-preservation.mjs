...
const dir=path.resolve('../artifacts'),baseline=path.join(dir,'v65-qa-preservation-before.json');
try{
 ...
 if(process.argv.includes('--compare')){const before=JSON.parse(fs.readFileSync(baseline,'utf8'));report.source_database_unchanged=JSON.stringify(before.hashes)===JSON.stringify(hashes);fs.writeFileSync(path.join(dir,'v65-qa-preservation-after.json'),JSON.stringify(report,null,2));if(!report.source_database_unchanged)throw Error('Dữ liệu nội dung/lịch sử đã đổi sau QA');}
